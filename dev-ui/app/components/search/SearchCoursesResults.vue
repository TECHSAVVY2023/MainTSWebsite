  ...
  >
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5 pb-4 border-b border-accent-purple/15">
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <h2 class="text-lg font-bold text-white m-0">Courses</h2>
    ...
    <ul class="list-none p-0 m-0 [&>li]:mb-2 [&>li:last-child]:mb-0">
      <li v-for="(item, i) in displayedItems" :key="i">
        ...
        >
          ...
          <span class="text-[0.8125rem] text-white/50 shrink-0">{{ item.instructor }} · {{ item.duration }}</span>
...
defineProps<{
  items: { slug: string; title: string; instructor: string; duration: string }[]
  displayedItems: { slug: string; title: string; instructor: string; duration: string }[]
